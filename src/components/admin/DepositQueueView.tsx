...
import { ArrowDownLeft, CheckCircle2, XCircle, Clock, Search, Download } from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-6 rounded-3xl bg-slate-900 border border-slate-800">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">
            Verifikasi dan setujui transaksi deposit QRIS/Bank dari member investor. Saldo member akan otomatis bertambah saat disetujui.
          </p>
        </div>
      ...
      <div className="p-4 rounded-2xl bg-slate-900 border border-slate-800 flex flex-col md:flex-row items-center gap-3">
        <div className="relative flex-1 w-full">
          ...
          <input
            type="text"
            placeholder="Cari ID transaksi, user ID, atau catatan..."
            value={search}
                ...
                filteredDeposits.map((d) => (
                  <tr key={d.id} className="hover:bg-slate-800/40 transition-colors">
                    ...
                    <td className="py-3.5 px-4 text-slate-300">
                      <span className="px-2 py-0.5 rounded bg-slate-800 text-slate-300 border border-slate-700 text-[11px] font-semibold">
